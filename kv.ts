...
export async function getLoops(): Promise<Loop[]> {
  const loops = await kv.get<Loop[]>(LOOPS_KEY)
  if (loops === null) {
    // First run — seed the store
    await kv.set(LOOPS_KEY, SEED_LOOPS)
    await kv.set(NOTES_KEY, SEED_NOTES)
    return SEED_LOOPS
  }
  return loops
}
...
export async function setLoops(loops: Loop[]): Promise<void> {
  await kv.set(LOOPS_KEY, loops)
}

export async function getNotes(): Promise<string[]> {
  const notes = await kv.get<string[]>(NOTES_KEY)
  return notes ?? []
}
